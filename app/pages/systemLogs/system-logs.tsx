  ...
  // 2. ⚡ OPERATION
  {
    ...
    render: (v) => {
      ...
      return (
        ...
        >
          <Text style={{ color: color, fontWeight: '700', fontSize: 11 }}>• {op}</Text>
        </Box>
      );
    },
  },
  ...
  return (
    <Box className="flex-1 bg-[#f8fafc]">
      ...
      <LinearGradient colors={['#2563EB', '#1D4ED8']} style={styles.header}>
        <Box className="flex-row items-center justify-between px-5 pb-3 pt-12">
          <Box>
        ...
        >
          {logs.length === 0 ? (
            <Box className="items-center justify-center py-20">
              <Text className="text-base text-typography-400">No system logs found</Text>
            </Box>
          ) : (
            <React.Fragment>
              <HtmlTable
        ...
        <Box style={styles.modalOverlay}>
          <Box style={styles.detailsContainer}>
            <Box style={styles.closeIconContainer}>
              ...
              <TouchableOpacity style={[styles.closeModalBtn]} onPress={() => setSelectedLog(null)}>
