...
const CONTACT_TABLE_COLUMNS: HtmlTableColumn<ContactRequest>[] = [
  ...
  },
  {
    ...
    render: (_v, row) => {
      ...
      const initials = name.split(" ").filter(Boolean).slice(0, 2).map((w) => w.charAt(0).toUpperCase()).join("") || "UN";
      ...
      return (
        <HStack space="sm" className="items-center">
          ...
          <VStack style={{ justifyContent: "center" }}>
            <Text style={{ fontSize: 13, fontWeight: "700", color: "#1e293b" }} numberOfLines={1}>{name}</Text>
          </VStack>
        </HStack>
      );
    },
  },
  {
    ...
    label: "Email",
    width: "180px",
    render: (v) => <Text style={{ fontSize: 13, fontWeight: "500", color: "#2563eb" }} numberOfLines={1}>{v || "—"}</Text>,
  },
  {
    key: "subject",
    label: "Reason",
    ...
    render: (v) => {
      ...
      return (
        <Box style={{ alignSelf: "flex-start", paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, backgroundColor: "#e5f0ff", borderWidth: 1, borderColor: "#193867" }}>
          <Text style={{ color: "#193867", fontWeight: "700", fontSize: 12 }} numberOfLines={1}>{val}</Text>
        </Box>
      );
    },
  },
  ...
  },
  {
    ...
    width: "130px",
    render: (v, row) => {
      const statusNum = v !== undefined ? Number(v) : (row as any).contact_status !== undefined ? Number((row as any).contact_status) : 0;
    ...
    render: (v, row) => {
      ...
      return (
        <HStack space="xs" className="items-center">
          <Box style={{ width: 24, height: 24, borderRadius: 12, backgroundColor: "#f1f5f9", alignItems: "center", justifyContent: "center" }}>
            <Text style={{ fontSize: 10, fontWeight: "700", color: "#64748b" }}>{String(name).charAt(0).toUpperCase()}</Text>
          </Box>
          <Text style={{ fontSize: 12, fontWeight: "500", color: "#475569" }} numberOfLines={1}>{name}</Text>
        </HStack>
      );
    },
  },
...
export default function ContactUsScreen() {
  ...
  const handleToggleStatus = async (item: ContactRequest) => {
    ...
    try {
      ...
      setRequests((prev) => prev.map((r) => ((r._id || r.id) === id ? { ...r, contactStatus: nextStatus } : r)));
    ...
    <Box className="flex-1 bg-[#f8fafc]">
      <LinearGradient colors={["#2563EB", "#1D4ED8"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.header}>
        ...
        <Box style={styles.headerContent}>
          ...
          <HStack style={{ justifyContent: "space-between", alignItems: "center", marginTop: 20 }}>
            <VStack style={{ flex: 1, paddingRight: 15 }}>
              <Heading style={styles.headerTitle}>Contact</Heading>
              <Text style={styles.headerSubtitle}>Read and resolve support and contact inquiries</Text>
            </VStack>
      ...
      <Box style={styles.mainCard}>
          ...
          <HStack space="sm" className="w-full">
            {(["all", "pending", "resolved"] as const).map((btn) => {
              ...
              return (
                <TouchableOpacity
                  key={btn}
                  style={[styles.tabBtn, isActive && styles.tabBtnActive, btn === "pending" && !isActive && styles.tabBtnPending, btn === "resolved" && !isActive && styles.tabBtnResolved]}
                  onPress={() => setFilter(btn)}
                >
                  <Text style={[styles.tabText, isActive && styles.tabTextActive, btn === "pending" && !isActive && styles.tabTextPending, btn === "resolved" && !isActive && styles.tabTextResolved]}>
        ...
        {loading ? (
          ...
          <ScrollView contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#193867" />}>
            {requests.length === 0 ? (
        ...
        <Box style={styles.modalOverlay}>
          <Box style={styles.modalContainer}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 400 }}>
              <VStack space="md">
                ...
                <Box>
                  ...
                  <Text style={styles.detailValue}>{selectedRequest?.first_name} {selectedRequest?.last_name}</Text>
                  <Text className="text-typography-500 text-xs mt-1">📧 {selectedRequest?.email}</Text>
                  {selectedRequest?.contact_no ? <Text className="text-typography-500 text-xs mt-0.5">📞 {selectedRequest?.contact_no}</Text> : null}
                </Box>
                <Box>
                  <Text style={styles.detailLabel}>Status</Text>
                  <Box style={[styles.statusBadge, { backgroundColor: selectedRequest?.contactStatus === 1 ? "#dcfce7" : "#fef9c3", alignSelf: "flex-start", marginTop: 4 }]}>
                    <Text style={{ color: selectedRequest?.contactStatus === 1 ? "#15803d" : "#a16207", fontSize: 10, fontWeight: "700" }}>{selectedRequest?.contactStatus === 1 ? "Resolved" : "Pending"}</Text>
                  </Box>
                </Box>
                <Box>
            ...
            <HStack space="sm" className="mt-6">
              <Button style={{ flex: 1 }} className="bg-primary-700 rounded-xl" onPress={() => selectedRequest && handleToggleStatus(selectedRequest)}>
                <ButtonText>{selectedRequest?.contactStatus === 1 ? "Mark Pending" : "Mark Resolved"}</ButtonText>
              </Button>
              <TouchableOpacity style={[styles.cancelBtn, { backgroundColor: "#fee2e2" }]} onPress={() => selectedRequest && handleDelete(selectedRequest)}>
...
const styles = StyleSheet.create({
  ...
  headerGlow: { position: "absolute", right: -50, top: -40, width: 220, height: 220, borderRadius: 110, backgroundColor: "rgba(255,255,255,0.08)" },
  backButton: { flexDirection: "row", alignItems: "center" },
  backIcon: { color: "#fff", fontSize: 20, marginRight: 8, fontWeight: "600" },
  backText: { color: "#fff", fontSize: 18, fontWeight: "600" },
  headerTitle: { color: "#fff", fontSize: 38, fontWeight: "700", marginBottom: 10 },
  headerSubtitle: { color: "rgba(255,255,255,0.88)", fontSize: 17, lineHeight: 24 },
  iconContainer: { width: 100, height: 100, borderRadius: 60, backgroundColor: "rgba(255,255,255,0.12)", justifyContent: "center", alignItems: "center" },
  headerEmoji: { fontSize: 64 },
  mainCard: { flex: 1, backgroundColor: "#ffffff", borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 16, paddingTop: 20, marginTop: -20, shadowColor: "#0f172a", shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.05, shadowRadius: 10, elevation: 4 },
  filterSection: { paddingBottom: 12, backgroundColor: "#fff", borderBottomWidth: 1, borderBottomColor: "#e2e8f0" },
  searchInput: { borderWidth: 1, borderColor: "#e2e8f0", borderRadius: 14, paddingHorizontal: 14, paddingVertical: 10, fontSize: 14, color: "#1e293b", backgroundColor: "#f8fafc", marginBottom: 8 },
  tabBtn: { flex: 1, alignItems: "center", justifyContent: "center", paddingVertical: 9, borderRadius: 20, backgroundColor: "#f1f5f9", borderWidth: 1, borderColor: "#e2e8f0" },
  ...
  closeIconContainer: { justifyContent: "space-between", borderRadius: 12, alignItems: "center", flexDirection: "row", marginBottom: 12 },
  closeModalBtn: { alignItems: "center", justifyContent: "center", padding: 8, borderRadius: 12, backgroundColor: "#f1f5f9" },
  ...
  messageBox: { borderWidth: 1, borderColor: "#e2e8f0", borderRadius: 12, padding: 12, backgroundColor: "#f8fafc", marginTop: 4 },
});