...
export default function ProfileScreen() {
  ...
  const refreshProfileData = async () => {
    ...
    setLoading(true);
    try {
      const res = await getProfile(authUserId);
      const normalized = normalizeProfile(res?.data || res);
      setProfileId(normalized.id || authUserId);
      setFirstName(normalized.first_name || user?.first_name || '');
      setLastName(normalized.last_name || user?.last_name || '');
      setContactNo(
        normalized.contact_no
          ? String(normalized.contact_no)
          : user?.contact_no
  ...
  const handlePickAndUploadImage = async () => {
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        const res = await uploadProfile(targetId, imageFile);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await updateProfile(targetId, {
